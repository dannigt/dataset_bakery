...
for f in $PREPRO_DIR/train/*eng*\.{src,trg}; do
	fname="${f##*/}"
	lan_pair="${fname%.*}"

	echo '*** Binarizing direction:' $lan_pair
	...
	if [[ "$f" == *src ]]; then
		...
		ln -s $f $PREPRO_DIR/train/$src_lang-$trg_lang.$trg_lang
		ln -s $f $PREPRO_DIR/train/$trg_lang-$src_lang.$trg_lang
	fi
...
# binarize dev set
for f in $PREPRO_DIR/dev/*\.{src,trg}; do
        fname="${f##*/}"
        lan_pair="${fname%.*}"
        ...
        if [[ "$f" == *src ]]; then
                ...
                ln -s $f $PREPRO_DIR/dev/$src_lang-$trg_lang.$trg_lang
                ln -s $f $PREPRO_DIR/dev/$trg_lang-$src_lang.$trg_lang
        fi
...
# binarize test set
for f in $PREPRO_DIR/test/*\.{src,trg}; do
        fname="${f##*/}"
        lan_pair="${fname%.*}"
	...
	if [[ $(wc -l <$f) -le 1000 ]]; then
		echo '*** Skipping test direction' $lan_pair 'because it has < 1000 sentences.'
	        ...
	        if [[ "$f" == *src ]]; then
        	        ...
        	        ln -s $f $PREPRO_DIR/test/$src_lang-$trg_lang.$trg_lang
                	ln -s $f $PREPRO_DIR/test/$trg_lang-$src_lang.$trg_lang
	        fi
